...
import { marketplaceHome, resolveDashboardContext, safeReturnPath, DashboardContextSchema } from './account-context.ts';
...
test('dual-capability accounts must choose context, never the first workspace', () => {
  assert.equal(marketplaceHome(access), '/settings');
  assert.equal(marketplaceHome({ ...access, workspaces: [] }), '/shifts');
  assert.equal(marketplaceHome({ ...access, worker: false, workspaces: [access.workspaces[0]] }), `/employer/shifts?workspace=${first}`);
